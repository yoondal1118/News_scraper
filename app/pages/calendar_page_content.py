...
def render():
    ...
    with col_y:
        year = st.selectbox("년도", options=[2024, 2025, 2026], index=2, key="cal_year_select")
        st.session_state["calendar_year"] = year
    with col_m:
        month = st.selectbox("월", options=list(range(1, 13)), index=st.session_state["calendar_month"] - 1, key="cal_month_select")
        st.session_state["calendar_month"] = month
    with col_info:
        st.write("")
        st.markdown(f"<div style='margin-top:25px;'>현재 선택: <span class='neon-text'>{year}년 {month}월</span></div>", unsafe_allow_html=True)
    ...
    render_bento_calendar(year, month, news_dates, issue_dates, on_day_click)
